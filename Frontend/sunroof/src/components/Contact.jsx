...
function Contact({userId,name,setName,email,setEmail,number,setNumber,step,setStep}) {
    ...
    const storeDetails = ()=>{
      if(name === "" || email === "" || number === ""){
        console.log("Feilds Can't be empty")
      }else{
        ...
        if(!isValidEmail(email)){
          ...
          try{
              axios.post('http://localhost:8000/api/store-details', {user_id:userId,name:name,email:email,number:number,})
                  .then(response =>{
  ...
  return (
    <div className='w-screen lg:w-[50rem] lg:px-[6rem] py-[3rem]'>
        ...
        <div className="flex mt-5 gap-2">
            ...
            <button onClick={()=>{storeDetails()}} className='bg-[#03a9f4] w-full text-white text-xl font-bold h-[3rem]'>UNLOCK ESTIMATE</button>
        </div>
    </div>
  )
}
